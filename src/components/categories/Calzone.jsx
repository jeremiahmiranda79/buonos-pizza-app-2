...
function MyVerticallyCenteredModal(props) {
  return (
    <Modal {...props} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
      ...
      <Modal.Body>
        <h4>Filling choices</h4>
        <h6>Calzone fillings are sold by the each</h6>
        <ul>{CALZONE_FILLINGS.map((filling) => (<li key={filling.id}>{filling.name} - Small: ${filling.priceSmall.toFixed(2)} Large: ${filling.priceLarge.toFixed(2)}</li>))}</ul>
      </Modal.Body>
    </Modal>
  );
};
...
const Calzone = () => {
  ...
  return (
    <Container id={PRODUCT_CATEGORIES[6].id} style={{scrollMarginTop: '150px'}}>
      <h1 className='Title' style={{marginTop: '10px', marginBottom: '10px'}}>{PRODUCT_CATEGORIES[6].name}</h1>
      <Image style={{marginBottom: '10px'}} src="/images/categories/calzone.webp" fluid alt={PRODUCT_CATEGORIES[6].name}/>
          ...
          <Col key={item.id}>
            <Card className='h-100'>
              ...
              <Card.Body>
                ...
                <Button style={{marginTop: '10px'}} variant="primary" onClick={() => setModalShow(true)}>See filling choices</Button>
