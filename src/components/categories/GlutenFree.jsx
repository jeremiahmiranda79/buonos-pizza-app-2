  ...
  return (
    <Modal {...props} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
      <Modal.Header closeButton>
        <Modal.Title id="contained-modal-title-vcenter">GLUTEN FREE TOPPINGS</Modal.Title>
      </Modal.Header>      
      <Modal.Body>
        <h4>Topping choices</h4>
        <h6>Gluten free pizza toppings are sold by the each</h6>
        <ul>{TOPPINGS_PIZZA.map((topping) => (<li key={topping.id}>{topping.name} - ${topping.priceGlutenFree.toFixed(2)}</li>))}</ul>        
      </Modal.Body>
    </Modal>
  );
};
